    ...
    while True:
        ...
        if command == 'restart':
            ...
            if restart(serverName):
                connection.sendall(f'{serverName} restarted successful \r\n'.encode())
            else:
                connection.sendall(f'{serverName} restarted with error. Check server container manager \r\n'.encode())
        if command == 'stop':
            serverName = dataReceived[1].rstrip()
            dockerModule = importlib.import_module("dockerManager")
            stop = getattr(dockerModule, "stopContainer")
            if  stop(serverName):
                connection.sendall(f'{serverName} successfully stopped \r\n'.encode())
            else:
                connection.sendall(f'{serverName} stopped with error. Check server container manager \r\n'.encode())
        if command == 'start':
            serverName = dataReceived[1].rstrip()
            dockerModule = importlib.import_module("dockerManager")
            start = getattr(dockerModule, "startContainer")
            if start(serverName):
                connection.sendall(f'{serverName} successfully started \r\n'.encode())
            else:
                connection.sendall(f'{serverName} started with error. Check server container manager \r\n'.encode())
        if command == 'remove':
            serverName = dataReceived[1].rstrip()
            directoryName = os.path.join(serverFolder, serverName)
            dockerModule = importlib.import_module("dockerManager")
            ...
            if remove(serverName):
                connection.sendall(f'{serverName} successfully remove \r\n'.encode())
            else:
                connection.sendall(f'{serverName} removed with error. Check server container manager \r\n'.encode())
                return False
